...
const updateData = (comic) => {
    form.genre = genre.value ? genre.value.id : comic.comic_genre_id;
    form.publisher = publisher.value
        ? publisher.value.id
        : comic.comic_publisher_id;
    form.library = library.value ? library.value.id : comic.library_id;
    form.title = comic.title;
    form.author = comic.author;
    form.stock = comic.stock;
    form.price = comic.price;
    form.put(route("comics.update", comic.id), {
        errorBag: "updateData",
        preserveScroll: true,
        onError: () => {
...
<template>
    <AppLayout title="Comics">
        ...
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            <FormSection @submitted="updateData(comic)">
                <template #title>Edit Comic</template>

                <template #description> Edit a comic data. </template>

                <template #form>
                    <div class="col-span-6 sm:col-span-4">
                        <InputLabel for="type" value="Comic Genre" />
                        <div class="dropdown relative mt-2 w-full">
                            <button
                                ref="genreInput"
                                class="dropdown-toggle w-full px-6 py-3 bg-teal-800 text-white font-medium text-sm leading-tight capitalize rounded shadow-md hover:bg-teal-400 text-white hover:shadow-lg focus:bg-teal-400 text-white focus:shadow-lg focus:outline-none focus:ring-0 active:bg-rose-200 active:shadow-lg transition duration-150 ease-in-out flex items-center whitespace-nowrap"
                                type="button"
                                id="genre"
                            ...
                            >
                                {{
                                    genre ? genre.name : comic.comic_genre_name
                                }}
                    ...
                    <div class="col-span-6 sm:col-span-4">
                        <InputLabel for="type" value="Comic Publisher" />
                        <div class="dropdown relative mt-2 w-full">
                            <button
                                ref="publisherInput"
                                class="dropdown-toggle w-full px-6 py-3 bg-teal-800 text-white font-medium text-sm leading-tight capitalize rounded shadow-md hover:bg-teal-400 text-white hover:shadow-lg focus:bg-teal-400 text-white focus:shadow-lg focus:outline-none focus:ring-0 active:bg-rose-200 active:shadow-lg transition duration-150 ease-in-out flex items-center whitespace-nowrap"
                    ...
                    <div class="col-span-6 sm:col-span-4">
                        <InputLabel for="type" value="Library" />
                        <div class="dropdown relative mt-2 w-full">
                            <button
                                ref="libraryInput"
                                class="dropdown-toggle w-full px-6 py-3 bg-teal-800 text-white font-medium text-sm leading-tight capitalize rounded shadow-md hover:bg-teal-400 text-white hover:shadow-lg focus:bg-teal-400 text-white focus:shadow-lg focus:outline-none focus:ring-0 active:bg-rose-200 active:shadow-lg transition duration-150 ease-in-out flex items-center whitespace-nowrap"
                            ...
                            >
                                {{
                                    library ? library.name : comic.library_name
                                }}
                                <div class="w-full flex justify-end">
                                    <svg
                                        aria-hidden="true"
